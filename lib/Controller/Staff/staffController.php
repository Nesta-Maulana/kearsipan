<?php
	...
	switch ($aksi) {
		case 'tambah-surat':
			...
			if ($jenis_id=='1') 
			{
				$redirect 		= baseurl('staff/surat-masuk');
			...
			elseif ($jenis_id=='2') 
			{
				$redirect 		= baseurl('staff/surat-keluar');
				$cekkodesurat = mysql_num_rows(mysql_query("SELECT * FROM surat where surat_kode='$surat_kode'"));
				if ($cekkodesurat < 1) 
				{
		...
		case 'input-head-disposisi':
			...
			if (mysql_num_rows($cek)>0) 
			{
				...
				header('location:'.baseurl('staff/disposisi-surat/').base64_encode($disposition->id));
			}
			else
			{
				...
				if ($jalan > 0) 
				{
					...
					header('location:'.baseurl('staff/disposisi-surat/').base64_encode($head->id));
				}
			}
		break;
		case 'input-detail-disposisi':
			...
			if (empty($disposisi_untuk)||empty($deskripsi)||empty($status_id)) 
			{
				...
				header('location:'.baseurl('staff/disposisi-surat/'.base64_encode($disposisi_id)));
			}
			else
			{
				...
				if ($update == true) 
				{
					...
					if ($akhir>$cekawal) 
					{
						...
						header('location:'.baseurl('staff/lihat-surat/'.base64_encode($sql->surat_kode)));
					}
				}
				else
				{
		...
		case 'batal-disposisi':
			...
			if ($jalan) 
			{
				...
				header('location:'.baseurl('staff/lihat-surat/'.base64_encode($surat->surat_kode)));
			}
		break;
		case 'filter-tampil':
			$filter 	= $_POST['filter'];
			if ($filter == 'View') 
			{
				...
				header('location:'.baseurl('staff/arsip-perusahaan'));
			}
			if ($filter == 'PDF') 
			{
				$_SESSION['filter'] 		= 'pdf';
		...
		case 'klik-notifikasi':
			...
			mysql_query("UPDATE disposisi_detail SET waktu_lihat='$date' WHERE id='$surat_kode'");
			//echo"UPDATE disposisi_detail SET waktu_lihat='$date' WHERE id='$surat_kode'";
			$diposisi_detail = mysql_fetch_object(mysql_query("SELECT * FROM disposisi_detail WHERE id='$surat_kode'"));
			$disposisi = mysql_fetch_object(mysql_query("SELECT * FROM disposisi where id='$diposisi_detail->disposisi_id'"));
			$surat = mysql_fetch_object(mysql_query("SELECT * FROM surat where id='$disposisi->surat_id'"));
			header('location:'.baseurl('staff/lihat-surat/'.base64_encode($surat->surat_kode)));
		break;
		case 'pilih-hak-akses':
			...
			header('location:'.baseurl('staff/kelola-pengguna')); 
		...
		case 'tambah-pengguna':
			...
			$status_account 	= base64_decode(antiInject($_POST['status_account'])); 
		...
		case 'edit-pengguna':
			...
			header('location:'.baseurl('staff/kelola-pengguna'.'/'.base64_encode($_SESSION['id'])));
		break;
		case 'ubah-pengguna':
			...
			{
				$_SESSION['aksi'] = "Username Sudah Terdaftar Sebagai Akun Lain";
				header('location:'.baseurl('staff/kelola-pengguna/'.$_GET['id']));				
			}
			else
			{
		...
		if ($cek>0) 
		{
			...
			header('location:'.baseurl('staff/kelola-jabatan'));
		}
		else
		{
			$id_user=$_POST['id_user'];
		...
		case 'edit-jabatan':
			...
			header('location:'.baseurl('staff/kelola-jabatan/'.$_GET['id']));
		break;
		case 'ubah-jabatan':
		$id = base64_decode($_GET['id']);
		$cek = mysql_num_rows(mysql_query("SELECT * FROM jabatan where jabatan='$_POST[jabatan]'"));
