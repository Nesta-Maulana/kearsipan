<?php
	...
	switch ($aksi) {
		case 'tambah-surat':
			...
			if ($jenis_id=='1') 
			{
				$redirect 		= baseurl('sekretaris/surat-masuk');
			...
			elseif ($jenis_id=='2') 
			{
				$redirect 		= baseurl('sekretaris/surat-keluar');
				$cekkodesurat = mysql_num_rows(mysql_query("SELECT * FROM surat where surat_kode='$surat_kode'"));
				if ($cekkodesurat < 1) 
				{
		...
		case 'input-head-disposisi':
			...
			if (mysql_num_rows($cek)>0) 
			{
				...
				header('location:'.baseurl('sekretaris/disposisi-surat/').base64_encode($disposition->id));
			}
			else
			{
				...
				if ($jalan > 0) 
				{
					...
					header('location:'.baseurl('sekretaris/disposisi-surat/').base64_encode($head->id));
				}
			}
		break;
		case 'input-detail-disposisi':
			...
			if (empty($disposisi_untuk)||empty($deskripsi)||empty($status_id)) 
			{
				...
				header('location:'.baseurl('sekretaris/disposisi-surat/'.base64_encode($disposisi_id)));
			}
			else
			{
				...
				if ($update == true) 
				{
					...
					if ($akhir>$cekawal) 
					{
						...
						header('location:'.baseurl('sekretaris/lihat-surat/'.base64_encode($sql->surat_kode)));
					}
				}
				else
				{
		...
		case 'batal-disposisi':
			...
			if ($jalan) 
			{
				...
				header('location:'.baseurl('sekretaris/lihat-surat/'.base64_encode($surat->surat_kode)));
			}
		break;
		case 'filter-tampil':
			$filter 	= $_POST['filter'];
			if ($filter == 'View') 
			{
				...
				header('location:'.baseurl('sekretaris/arsip-perusahaan'));
			}
			if ($filter == 'PDF') 
			{
				$_SESSION['filter'] 		= 'pdf';
		...
		case 'klik-notifikasi':
			...
			header('location:'.baseurl('sekretaris/lihat-surat/'.base64_encode($surat_kode)));
		break;
		case 'pilih-hak-akses':
			...
			header('location:'.baseurl('sekretaris/kelola-pengguna')); 
		...
		case 'edit-pengguna':
			...
			header('location:'.baseurl('sekretaris/kelola-pengguna'.'/'.base64_encode($_SESSION['id'])));
		break;
		case 'ubah-pengguna':
			...
			{
				$_SESSION['aksi'] = "Username Sudah Terdaftar Sebagai Akun Lain";
				header('location:'.baseurl('sekretaris/kelola-pengguna/'.$_GET['id']));				
			}
			else
			{
		...
		if ($cek>0) 
		{
			...
			header('location:'.baseurl('sekretaris/kelola-jabatan'));
		}
		else
		{
			$id_user=$_POST['id_user'];
		...
		case 'edit-jabatan':
			...
			header('location:'.baseurl('sekretaris/kelola-jabatan/'.$_GET['id']));
		break;
		case 'ubah-jabatan':
		$id = base64_decode($_GET['id']);
		$cek = mysql_num_rows(mysql_query("SELECT * FROM jabatan where jabatan='$_POST[jabatan]'"));
